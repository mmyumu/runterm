...
- **`src-tauri/src/desktop.rs`** (Windows-only): Tauri commands `load_config`, `save_config`, `list_distributions`, `list_directories`, `launch_projects` (project ids + mode `tabs`/`windows`; projects with `launchAll` feed the sidebar's "Tout lancer" buttons). Launch flow: check `wt.exe` and WSL distributions → for each project, `prepare`: resolve → verify each pane directory in WSL → `mktemp -d /tmp/runterm-*` with `umask 077` → write one script per Bash pane via stdin (each script deletes itself and tries to rmdir the folder on start) → if any project fails, remove the folders of all prepared projects (a project with a `host` skips the directory checks and temp folder: `prepare_ssh` puts each pane script, base64-encoded, into an `ssh -t` command run from WSL via `core::ssh_command`) → open the projects' `url` (`url_to_open`: empty when `urlDisabled`) in the default browser (`open_urls`: the `https` handler read from the registry with `reg.exe`, started once with every URL so they share a window, `core::browser_args` adding `--new-window` for Chromium-based browsers, else `rundll32.exe url.dll,FileProtocolHandler` per URL) → run `wt.exe` once (tabs) or once per project (windows) with args from `tabs_args`.
