...
18. Check **Ouvrir VS Code** on a WSL project with an empty **Dossier VS Code** and launch it: VS Code opens the project root connected to the distribution (`WSL: <distribution>` in the status bar), behind the Windows Terminal window. Set the folder to a subfolder, then to an absolute path containing a space, and relaunch: each opens that folder. Launch again without closing VS Code: the existing window comes to the front instead of a new one. On the SSH project, check it too: VS Code connects to the host (`SSH: <host>`) with Remote - SSH, using the Windows `~/.ssh/config`. Temporarily rename the VS Code folder: the launch fails with « Visual Studio Code est introuvable » and opens neither browser nor terminal.
19. Build the installer with `npm run tauri -- build`, install it and check startup and a WSL launch from the app shortcut.
